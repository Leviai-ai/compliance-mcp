...
interface Regulation {
  ...
  fullComplianceDate?: string;
  expectedEnactment?: string;
  description: string;
...
// Load data
const regulations: Regulation[] = loadJsonData("regulations.json");
const calendarEvents: CalendarEvent[] = loadJsonData("calendar.json");
const riskTemplates: RiskTemplate[] = loadJsonData("risk-templates.json");
const policyUpdates: PolicyUpdate[] = loadJsonData("policy-updates.json");
...
function isUpcoming(dateStr: string, daysAhead: number = 365): boolean {
  try {
  ...
  async ({ jurisdiction, category, status, search }) => {
    ...
    if (jurisdiction) {
      ...
      filtered = filtered.filter(r => 
        r.jurisdiction.toUpperCase() === normalizedJurisdiction ||
    ...
    if (category && category.length > 0) {
      filtered = filtered.filter(r =>
        category.some(c => r.category.includes(c.toLowerCase()))
      );
    }
...
server.tool(
  ...
  async ({ jurisdiction, days_ahead = 365, priority, regulation }) => {
    ...
    if (jurisdiction) {
      ...
      filtered = filtered.filter(event =>
        jurisdictionRegulations.includes(event.regulation)
      );
    }
    ...
    if (priority) {
      filtered = filtered.filter(event => 
        event.priority.toLowerCase() === priority.toLowerCase()
      );
    }
    ...
    if (regulation) {
      filtered = filtered.filter(event => 
        event.regulation.toLowerCase() === regulation.toLowerCase()
      );
    }
...
server.tool(
  ...
  async ({ regulation_id }) => {
    const reg = regulations.find(r => 
      r.id.toLowerCase() === regulation_id.toLowerCase() ||
    ...
    // Dates section
    const dates = [];
    if (reg.enactmentDate) dates.push(`⚡ **Enacted:** ${formatDate(reg.enactmentDate)}`);
    if (reg.applicabilityDate) dates.push(`📅 **Applicable:** ${formatDate(reg.applicabilityDate)}`);
    if (reg.fullComplianceDate) dates.push(`⚠️  **Full Compliance:** ${formatDate(reg.fullComplianceDate)}`);
    if (reg.expectedEnactment) dates.push(`🔮 **Expected:** ${formatDate(reg.expectedEnactment)}`);
...
server.tool(
  ...
  async ({ use_case, jurisdictions, custom_description }) => {
    const template = riskTemplates.find(t => 
      t.useCase.toLowerCase() === use_case.toLowerCase() ||
    ...
    if (template) {
      ...
      normalizedJurisdictions.forEach(jurisdiction => {
        const relevantRegs = regulations.filter(r => 
          r.jurisdiction.toUpperCase().includes(jurisdiction) ||
...
server.tool(
  ...
  async ({ jurisdiction, regulation, days_back = 90, impact_level }) => {
    ...
    filtered = filtered.filter(update => 
      new Date(update.date) >= cutoffDate
    );
    ...
    if (jurisdiction) {
      ...
      filtered = filtered.filter(update =>
        jurisdictionRegulations.includes(update.regulation) ||
    ...
    if (regulation) {
      filtered = filtered.filter(update => 
        update.regulation.toLowerCase() === regulation.toLowerCase() ||
    ...
    if (impact_level) {
      filtered = filtered.filter(update => 
        update.impact.toLowerCase() === impact_level.toLowerCase()
      );
    }
...
server.tool(
  ...
  async ({ jurisdictions, focus_area }) => {
    ...
    const jurisdictionData = normalizedJurisdictions.map(jurisdiction => {
      const jurisdictionRegs = regulations.filter(r => 
        r.jurisdiction.toUpperCase().includes(jurisdiction) ||
        r.jurisdiction.toUpperCase() === jurisdiction
      );
        ...
        jd.regulations.forEach(reg => {
          ...
          if (focus_area) {
            ...
            if (focusAreaLower === "penalties" && reg.penalties) {
              ...
              if (reg.enactmentDate) sections.push(`📅 Enacted: ${formatDate(reg.enactmentDate)}`);
              if (reg.applicabilityDate) sections.push(`📅 Applicable: ${formatDate(reg.applicabilityDate)}`);
              if (reg.fullComplianceDate) sections.push(`⚠️  Full Compliance: ${formatDate(reg.fullComplianceDate)}`);
            ...
            // General overview
            if (reg.enactmentDate) sections.push(`📅 Enacted: ${formatDate(reg.enactmentDate)}`);
            if (reg.penalties) sections.push(`⚖️  Penalties: ${reg.penalties}`);
